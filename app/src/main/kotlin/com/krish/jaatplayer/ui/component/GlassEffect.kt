...
import com.krish.jaatplayer.ui.component.backdrop.Backdrop
import com.krish.jaatplayer.ui.component.backdrop.drawBackdrop
import com.krish.jaatplayer.ui.component.backdrop.effects.blur
import com.krish.jaatplayer.ui.component.backdrop.effects.colorControls
import com.krish.jaatplayer.ui.component.backdrop.effects.lens
import com.krish.jaatplayer.ui.component.backdrop.highlight.Highlight
import com.krish.jaatplayer.ui.component.backdrop.shadow.Shadow

/**
 * User-configurable parameters of the liquid glass effect, sourced from DataStore
 * preferences in [com.krish.jaatplayer.MainActivity] and distributed through
...
data class GlassEffectConfig(
    ...
    val adoptThemeColor: Boolean = true,
...
/** Visual reference style for the Liquid Glass dock and per-menu glass surfaces. */
enum class GlassStyle {
    /** Apple iOS 26 Liquid Glass: lens refraction, chromatic aberration, specular rim, low tint. */
    APPLE,
    /** Material 3 glassmorphism: flatter frosted-glass look, no lens/chromatic aberration, higher tint. */
    MATERIAL3;

    companion object {
        fun fromPref(value: String): GlassStyle = if (value == "material3") MATERIAL3 else APPLE
    }
}
...
/**
 * Dedicated Liquid Glass parameters for the dock/menus, separate from the
 * Player/MiniPlayer/NavBar surfaces so they can be tuned independently.
 */
@Stable
data class MenuGlassConfig(
    val style: GlassStyle = GlassStyle.APPLE,
    val blurRadius: Float = 12f,
    val vibrancy: Float = 1f,
    val lensHeight: Float = 0.5f,
    val lensAmount: Float = 0.5f,
    val adoptThemeColor: Boolean = true,
    val playerMenuEnabled: Boolean = true,
    val queueMenuEnabled: Boolean = true,
    val qualityMenuEnabled: Boolean = false,
    ...
    fun isEnabledFor(menu: GlassMenu): Boolean = when (menu) {
        ...
        GlassMenu.QUEUE -> queueMenuEnabled
        GlassMenu.QUALITY -> qualityMenuEnabled
    ...
    fun toGlassEffectConfig(globalEnabled: Boolean): GlassEffectConfig = GlassEffectConfig(
        globalEnabled = globalEnabled,
        vibrancy = vibrancy,
        blurRadius = blurRadius,
        lensHeight = if (style == GlassStyle.MATERIAL3) 0f else lensHeight,
        lensAmount = if (style == GlassStyle.MATERIAL3) 0f else lensAmount,
        chromaticAberration = style == GlassStyle.APPLE,
        depthEffect = style == GlassStyle.APPLE,
        surfaceOpacity = if (style == GlassStyle.MATERIAL3) 0.55f else 0.3f,
        adoptThemeColor = adoptThemeColor,
    )
}

val LocalMenuGlassConfig = staticCompositionLocalOf { MenuGlassConfig() }
...
fun Modifier.liquidGlass(
    ...
    } else if (!config.adoptThemeColor) {
        // "Natural" glass: truly neutral clear glass without any color overlay.
        Color.Transparent
    ...
    return drawBackdrop(
        ...
        highlight = if (applyEdgeEffects && config.adoptThemeColor) ({ Highlight.Default }) else null,
        shadow = if (applyEdgeEffects && config.adoptThemeColor) ({ Shadow.Default }) else null,
        onDrawSurface = {
