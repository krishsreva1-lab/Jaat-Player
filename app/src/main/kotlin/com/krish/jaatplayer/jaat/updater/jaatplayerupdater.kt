...
package com.krish.jaatplayer.jaat.updater
...
import com.krish.jaatplayer.jaat.updater.downloadmanager.UpdateDownloadWorker
import com.krish.jaatplayer.jaat.updater.downloadmanager.DownloadNotificationManager
...
import com.krish.jaatplayer.ui.component.ChangelogItem
import com.krish.jaatplayer.ui.component.leadingItemShape
import com.krish.jaatplayer.ui.component.middleItemShape
import com.krish.jaatplayer.ui.component.endItemShape
import com.krish.jaatplayer.ui.component.detachedItemShape
import com.krish.jaatplayer.ui.component.parseMarkdown
import com.krish.jaatplayer.ui.component.endItemShape
import com.krish.jaatplayer.ui.component.detachedItemShape
import com.krish.jaatplayer.ui.component.AnimatedActionButton
import com.krish.jaatplayer.ui.component.ExpressiveIconButton
import com.krish.jaatplayer.ui.component.ErrorSnackbar
...
sealed class JaatUpdateStatus {
    object Idle : JaatUpdateStatus()
    object Checking : JaatUpdateStatus()
    ...
    data class NoUpdate(val version: String) : JaatUpdateStatus()
    data class Error(val message: String) : JaatUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<JaatUpdateStatus>(JaatUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = JaatUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        JaatUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = JaatUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is JaatUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as JaatUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is JaatUpdateStatus.Idle, is JaatUpdateStatus.Checking, is JaatUpdateStatus.NoUpdate, is JaatUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is JaatUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is JaatUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/krishsreva1-lab/Jaat-Player/releases/download/${currentStatus.version}/jaatplayer.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is JaatUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is JaatUpdateStatus.Checking -> {
                                JaatLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is JaatUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/krishsreva1-lab/Jaat-Player/releases/latest")
            ...
            if (shouldShow) {
                ...
                try {
                    val changelogUrl =
                        URL("https://github.com/krishsreva1-lab/Jaat-Player/releases/download/$tagWithPrefix/changelog.json")
